class ChecksController < ApplicationController
  ...
  def update
    ...
    if @check.updated_at.strftime('%-m月%-d日') == Date.today.strftime('%-m月%-d日')
      ...
      redirect_to root_path if @check_update.save
    end
  end
  ...
  def destroy
    @checks = Check.where(user_id: current_user.id)
    @check = @checks.last
    redirect_to root_path if @check.destroy
  end
  ...
  def show
    ...
    if @user_all_completes.present?
      @user_all_completes.each do |complete|
        @complete = complete if @check.updated_at.strftime('%-m月%-d日') == complete.updated_at.strftime('%-m月%-d日')
      end
    end
  end
